  ...
  return (
    <header className="sticky top-0 z-50">
      ...
      <div className="bg-[#F5F5F5] border-b border-gray-200">
        <div className="container-custom flex items-center h-9 justify-between">
          {/* Mobile Menu Toggle (NEW LOCATION) */}
          <button 
            onClick={() => setIsMobileMenuOpen(true)}
            className="md:hidden p-2 -ml-2 text-[#555] hover:text-[#1A142E] transition-colors"
          >
      ...
      <nav className="bg-white border-b border-gray-100">
        <div className="container-custom flex items-center h-[62px] sm:h-[78px]">
          
          {/* Mobile Menu Toggle moved — keeping space for desk content */}

          {/* Logo — matches reference closely */}
          <Link href="/" className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
            <svg viewBox="0 0 48 48" className="w-[30px] h-[30px] sm:w-[46px] sm:h-[46px] flex-shrink-0" fill="none">
            ...
            <div className="leading-[1.15]">
              <div className="text-[13px] sm:text-[20px] font-black tracking-[0.06em] text-[#1A142E] uppercase">{title}</div>
              <div className="text-[6.5px] sm:text-[9.5px] font-semibold tracking-[0.22em] text-[#8B7BB4] uppercase">Boutique Store</div>
            </div>
          </Link>
          ...
          <div className="flex items-center gap-3 sm:gap-5 flex-shrink-0 ml-auto">
            ...
            {isLoggedIn ? (
              <Link href="/profile" className="text-[#555] hover:text-[#C8382A] transition-colors">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} className="w-[20px] h-[20px] sm:w-[24px] sm:h-[24px]">
                  <path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2" /><circle cx="12" cy="7" r="4" />
                </svg>
              </Link>
            ) : (
              <div className="flex items-center gap-1.5 sm:gap-2 text-[9.5px] sm:text-[13px] text-[#555]">
            ...
            <Link href="/wishlist" className="relative text-gray-500 hover:text-[#C8382A] transition-colors">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} className="w-[19px] h-[19px] sm:w-[24px] sm:h-[24px]">
                <path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z" />
              </svg>
              {wishlist.length > 0 && (
                <span className="absolute -top-1.5 -right-1.5 min-w-[14px] h-[14px] sm:min-w-[16px] sm:h-[16px] rounded-full bg-[#8B7BB4] text-[7.5px] sm:text-[8px] font-black text-white flex items-center justify-center border border-white px-0.5 animate-in zoom-in duration-200">
                  {wishlist.length}
                </span>
              )}
            </Link>
            ...
            <Link href="/cart" className="relative text-gray-500 hover:text-[#C8382A] transition-colors">
              ...
              {cartCount > 0 && (
                <span className="absolute -top-1.5 -right-1.5 min-w-[14px] h-[14px] sm:min-w-[16px] sm:h-[16px] rounded-full bg-[#C8382A] text-[7.5px] sm:text-[8px] font-black text-white flex items-center justify-center border border-white px-0.5 animate-in zoom-in duration-200">
                  {cartCount}
                </span>
              )}
            </Link>
